...
gene_coexp_cor <- function(disease_name){
  df = dplyr::filter(octad_cell_line_meta, disease == disease_name)
  cell_lines = rownames(df)
  exp_features = as.character(octad_cell_line_features$id[octad_cell_line_features$type == "expression"])
  data = octad_cell_line_matrix[cell_lines, exp_features]
  cor.test(data[, "expression_ERBB2"], data[, "expression_AKT1"], method = "pearson")
}

gene_coexp_cor('Breast Cancer')
gene_coexp_cor('Pancreatic Cancer')
...
library(dplyr)
library(ggplot2)
df = dplyr::filter(octad_cell_line_meta, disease == 'Lung Cancer' | disease == 'Colon/Colorectal Cancer' | disease == 'Pancreatic Cancer')
cell_lines = rownames(df)
exp_features = as.character(octad_cell_line_features$id[octad_cell_line_features$type == "sensitivity"])
data = octad_cell_line_matrix[cell_lines, exp_features]
na_samples = apply(data, 1, function(x){sum(is.na(x))})
data = data[na_samples == 0, ]
dim(data)
